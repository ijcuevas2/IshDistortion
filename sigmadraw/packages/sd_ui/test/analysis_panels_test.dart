...
void main() {
  ...
  group('NyquistPanel', () {
    testWidgets('prompts for source/sink when neither is present', (
      ...
      final doc = createBlankSdDocument();
      await tester.pumpWidget(_wrap(NyquistPanel(document: doc)));
      expect(find.textContaining('Place a "source"'), findsOneWidget);
    });
    ...
    testWidgets('a pure gain chain plots without error', (tester) async {
      ...
      expect(tester.takeException(), isNull);
      expect(find.byType(CustomPaint), findsWidgets);
      expect(find.textContaining('-1 reference point'), findsOneWidget);
    });
  ...
  group('SpectrogramPanel', () {
    testWidgets('prompts for source/sink when neither is present', (
      tester,
    ) async {
      final doc = createBlankSdDocument();
      await tester.pumpWidget(_wrap(SpectrogramPanel(document: doc)));
      expect(find.textContaining('Place a "source"'), findsOneWidget);
    });
  ...
  group('GroupDelayPanel', () {
    testWidgets('prompts for source/sink when neither is present', (
      tester,
    ) async {
      final doc = createBlankSdDocument();
      await tester.pumpWidget(_wrap(GroupDelayPanel(document: doc)));
      expect(find.textContaining('Place a "source"'), findsOneWidget);
    });
  ...
  group('ImpulseStepPanel', () {
    testWidgets('prompts for source/sink when neither is present', (
      tester,
    ) async {
      final doc = createBlankSdDocument();
      await tester.pumpWidget(_wrap(ImpulseStepPanel(document: doc)));
      expect(find.textContaining('Place a "source"'), findsOneWidget);
    });
  ...
  group('RootLocusPanel', () {
    testWidgets('prompts for source/sink when neither is present', (
      tester,
    ) async {
      final doc = createBlankSdDocument();
      await tester.pumpWidget(_wrap(RootLocusPanel(document: doc)));
      expect(find.textContaining('Place a "source"'), findsOneWidget);
    });
    ...
    testWidgets('a fully concrete gain chain shows "nothing to sweep", not '
        'a bogus plot', (tester) async {
      final doc = createBlankSdDocument();
      doc.root.appendChild(source.instantiate(instanceId: 'src', x: 0, y: 0));
      doc.root.appendChild(
        gain.instantiate(instanceId: 'g1', params: {'gain': 2.0}, x: 100, y: 0),
      ...
      expect(tester.takeException(), isNull);
      expect(find.textContaining('Nothing to sweep'), findsOneWidget);
    });

    testWidgets('a chain with exactly one symbolic gain sweeps it and '
        'plots without error', (tester) async {
      final doc = createBlankSdDocument();
      doc.root.appendChild(source.instantiate(instanceId: 'src', x: 0, y: 0));
      doc.root.appendChild(
        gain.instantiate(instanceId: 'g1', params: {'gain': 'k'}, x: 100, y: 0),
      ...
      expect(tester.takeException(), isNull);
      expect(find.byType(CustomPaint), findsWidgets);
      expect(find.textContaining('Sweeping "k"'), findsOneWidget);
    });

    testWidgets('a chain with two different symbolic gains names both in '
      ...
      doc.root.appendChild(
        gain.instantiate(instanceId: 'g2', params: {'gain': 'm'}, x: 150, y: 0),
      );
      doc.root.appendChild(sink.instantiate(instanceId: 'snk', x: 200, y: 0));
      doc.root.appendChild(
        SdElement(const SdQName('path'))
      ...
      expect(find.textContaining('needs exactly one'), findsOneWidget);
